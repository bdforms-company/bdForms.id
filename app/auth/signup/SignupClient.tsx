      ...
      <div className="glass w-full max-w-md rounded-2xl p-8">
        ...
        {success ? <p className="text-center text-sm" style={{ color: "var(--success)" }}>✅ Silakan cek email Anda untuk verifikasi akun.</p> : (
          <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
            ...
            <button type="submit" disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-xl py-3 font-bold shadow-md transition hover:-translate-y-0.5 disabled:translate-y-0 disabled:opacity-50" style={{ background: "var(--primary)", color: "var(--on-primary)" }}>{loading && <span className="material-symbols-outlined animate-spin text-lg">progress_activity</span>}{loading ? "Memproses..." : "Daftar"}</button>
        ...
        <p className="mt-6 text-center text-sm" style={{ color: "var(--on-surface-variant)" }}>Sudah punya akun? <Link href="/auth/login" className="font-semibold" style={{ color: "var(--primary)" }}>Masuk</Link></p>
      </div>
    </div>
  );
}
