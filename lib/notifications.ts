...
export function generateNotifications(events: EventLike[]): Notification[] {
  ...
  events.forEach((event) => {
    ...
    if (event.package_status === "pending_payment") notifications.push({ id: `pending-${event.id}`, type: "warning", title: "Menunggu Konfirmasi Pembayaran", message: `Event "${event.name}" menunggu konfirmasi pembayaran dari tim Regesit.`, eventId: String(event.id), read: false, createdAt: new Date(event.created_at ? String(event.created_at) : now) });
